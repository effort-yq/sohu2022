...
def get_data(data_path,datatype):
    ...
    if datatype == 'train':
        with open(data_path, 'r', encoding='utf-8') as f:
            for dict_txt in json.load(f):

                if dict_txt['content'] == '':
                    continue
                for j in dict_txt['spo_list']:

                    content.append(dict_txt['content'])
                    entity.append(j[0])
                    label.append(j[-1])

        data = pd.DataFrame({'content': content, 'entity': entity, 'label': label})
        ...
        data=pd.DataFrame({'text_id': text_id,'content':content, 'entity': entity})
