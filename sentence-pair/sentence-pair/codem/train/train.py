...
def main():
    ...
    if args.pretrain:
        ...
        for i in range(3):
            train_data = pd.read_csv('../utils/kfold/data_KFold_24/data{}/train.csv'.format(i))
            dev_data = pd.read_csv('../utils/kfold/data_KFold_24/data{}/dev.csv'.format(i))
            train_data['label'] = train_data['label'].apply(lambda x: pp_dict[x])
            dev_data['label'] = dev_data['label'].apply(lambda x: pp_dict[x])

            ut.simmodels(model_path,
                         save_path+f'/{i}',
                         model_type=args.model_type,).train(train_data, dev_data)

if __name__=='__main__':
    if args.pretrain:
        model_path = r'/home/huangyongqing/ali_pytorch/WenTianSearch-main/WenTianSearch-main/chinese-roberta-wwm-ext'
        save_path = './model_param/pretrained_model_param/'
    else:
        model_path = r'/home/huangyongqing/ali_pytorch/WenTianSearch-main/WenTianSearch-main/chinese-roberta-wwm-ext'
        save_path = './model_param/saved_model/'
